...
def ccf_metric(x, y, lag=1):
    ccf_x = cacf_torch(x, lags=6)
    ccf_y = cacf_torch(y, lags=6)
...
def cacf_torch(x, lags: list, dim=(0, 1)):
    ...
    cacf_list = list()
    std = torch.std(x_l, unbiased=False, dim=(0, 1)) * torch.std(
        x_r, unbiased=False, dim=(0, 1)
    )
    for i in range(lags):
...
def Sig_mmd(X, Y, depth):
    ...
    X = AddTime(X).detach().cpu().numpy()
    Y = AddTime(Y).detach().cpu().numpy()
    n_components = 20
